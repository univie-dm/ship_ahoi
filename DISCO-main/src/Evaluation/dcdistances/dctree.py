...
class DCTree:
    ...
    def __repr__help(
        ...
        left_content = f"{self.__repr__help(node.left, pointer_left, padding_for_both, node.right is not None)}"
        right_content = f"{self.__repr__help(node.right, pointer_right, padding_for_both, False)}"
        region_end = f"\n   {padding.replace('|', ' ')}// #endregion"
        
        return region_start + tree_content + left_content + right_content + region_end
