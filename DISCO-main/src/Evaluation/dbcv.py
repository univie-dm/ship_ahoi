# Implementation of DBCV by
# - Author: https://hdbscan.readthedocs.io/en/latest/api.html#id128
# - Source: https://github.com/scikit-learn-contrib/hdbscan/blob/master/hdbscan/validity.py
# - License: BSD-3-Clause license (https://github.com/scikit-learn-contrib/hdbscan/blob/master/LICENSE)

# Paper: Density-Based Clustering Validation
# Authors:  Davoud Moulavi, Pablo A. Jaskowiak, Ricardo J. G. B. Campello, Arthur Zimek, and Jörg Sander
...
def distances_between_points(
    ...
    **kwd_args :
    ...
    if no_coredist:
        ...
        stacked_distances = np.dstack([distance_matrix, core_dist_matrix, core_dist_matrix.T])
...
def internal_minimum_spanning_tree(mr_distances):
    ...
    edge_selection = np.prod(
        np.in1d(min_span_tree.T[:2], vertices).reshape((min_span_tree.shape[0], 2), order="F"),
        axis=1,
    ).astype(bool)
...
def density_separation(
    ...
    **kwd_args :
    ...
    if no_coredist:
        ...
        mr_dist_matrix = np.dstack([distance_matrix, core_dist_matrix1, core_dist_matrix2]).max(
            axis=-1
        )
...
def validity_index(
    ...
    **kwd_args :
    ...
    labels_set = np.delete(labels_set, np.argwhere(labels_set == -1))
    lookup_cluster = dict(zip(labels_set, range(max_cluster_id)))
    lookup_cluster.update({-1: -1})
    sort_idx = np.argsort(list(lookup_cluster.keys()))
    idx = np.searchsorted(list(lookup_cluster.keys()), labels, sorter=sort_idx)
    labels = np.asarray(list(lookup_cluster.values()))[sort_idx][idx]
